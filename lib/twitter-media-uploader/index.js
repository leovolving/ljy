...
class TwitterMediaUploader {
    constructor() {
        this.mediaId = '1242909472210599936'
        this.mediaPath = null
        this.mediaSize = 0
        this.mediaFile = null
    ...
    async init(path, type) {
        ...
        this.mediaPath = path
        this.mediaFile = await this.getFile();
        this.mediaSize = this.mediaFile.length;
    ...
    async getFile() {
        if (!this.mediaPath.startsWith('http')) return fs.readFileSync(this.mediaPath);
    ...
    async processFile() {
        ...
        for (let start = 0; start < this.mediaSize; start += sliceSize) {
            ...
            const s = this.sliceFile(this.mediaFile, start, end);
